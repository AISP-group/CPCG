...
from datasets.BigCodeDataset import BigCodeGenDataset
...
# Obtain the content in the corresponding format
        ...
        try:
            ...
            if len(sample_io) > 0:
                ...
                if type(sample_io[0]) == dict:
                    result_lines = []
                    for io in sample_io:
                        input_str = io.get('input', '')
                        output_val = io.get('output', '')


                        if isinstance(output_val, (list, tuple)) and len(output_val) > 0:
                            output_str = output_val[0]
                        else:
                            output_str = str(output_val)

                        result_lines.append(f"Input:\n{input_str}\nExpected output:\n{output_str}")
            ...
            return f"Error processing sample_io: {str(e)}\nRaw data: {str(sample_io)}"
...
""",
        ...
        if type(self.data) == BigCodeGenDataset:
            sample_io_prompt = f"## Sample Test cases: \n{item['sample_io']}\n"

        # plan agent
        plannings = []
        for example_no, example in enumerate(response1["problem"], start=1):
            # for loop_num in range(0, self.k):
...
```
            ...
            if self.model.model_params['model'] == "Llama3.1-Instruct-8B":
                verification_res = remove_think(verification_res)
        ...
        for planning_with_ex in f_plans:
            planning, confidence = planning_with_ex

            file_path = './src/promptings/module_example'
...
"""
            ...
            if self.model.model_params['model'] == "DeepSeek-R1-Distill-Qwen-7B":
                module_res = remove_think(module_res)
            item['api_calls'] += 1
            # time.sleep(1)
            ...
            if type(self.data) == APPSDataset or type(self.data) == CodeContestDataset or type(
                ...
                file_path = './src/promptings/final_code_example'
                with open(file_path, 'r', encoding='utf-8') as infile:
                    c_example = infile.read()
            else:
                file_path = './src/promptings/final_code_example_n'
                with open(file_path, 'r', encoding='utf-8') as infile:
                    c_example = infile.read()
            final_code = f"""
