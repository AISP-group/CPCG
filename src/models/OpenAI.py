...
class GPT4(OpenAIModel):
    def prompt(self, processed_input: list[dict]):
        self.model_params["model"] = "deepseek-reasoner"
...
class DeepSeek(OpenAIModel):
    def prompt(self, processed_input: list[dict]):
        self.model_params["model"] = "deepseek-chat"
        self.model_params['temperature'] = 0.32
        return super().prompt(processed_input)

class DeepSeek_7B(OpenAIModel):
    def prompt(self, processed_input: list[dict]):
        self.model_params["model"] = "DeepSeek-R1-Distill-Qwen-7B"
        self.model_params['temperature'] = 1
        self.model_params["max_tokens"] = 8912
        return super().prompt(processed_input)
class Llama_8B(OpenAIModel):
    def prompt(self, processed_input: list[dict]):
        self.model_params["model"] = "Llama3.1-Instruct-8B"
        self.model_params['temperature'] = 1
        return super().prompt(processed_input)
